...
function checkLifeSpan(oldLifeSpan) {
  ...
  if (numberLifeSpan.ended === false) {
    ...
    if (numberLifeSpan.end === 0 && numberLifeSpan.ended === true) {
...
async function grabStuff(type, id, full, div) {
  if (busy === false) {
    busy = true;
    count = 0;
    let json = {}
    ...
    if (full === true) {
      switch (type) {
        ...
        case "event":
          ...
          break;
        default:
          throw new Error("add extra get stuff")
      }
    }
    ...
    for (let i = 0; i < member.length; i++) {
      if (member[i].parent.artist.length > 0) {
        ...
        for (let j = 0; j < artist.length; j++) {
          ...
          if (double > -1) {
            console.log("do nothing for doubles" + artist[j]);
          }
          else {
